...
\textbf{Dirichlet uniqueness (Poisson)}: Given a bounded domain $D\subseteq \mathbb{R}^2$, the Dirichlet problem has at most one solution $u\in C^2(D)\cap C(\overline{D})$.
\[
\begin{cases}
  \Delta u = f & \text{in D}\\
...
\textbf{Note:} This also applies to Laplace ($\Delta u = 0$)
