...
\end{cases}
\]
Assume that $\exists s_0 \in \mathbb{R}$ such that transversality cond. holds at $(0, s_0)$
det
\[
\begin{pmatrix}
...
\warning{If $f,g,F$ are odd/even/T-periodic in $x$, then $u(x,t)$ is odd/even/T-periodic, too (see 6.3/7.2)}
\hl{TODO}: What are the requirements for d'Alembert's formula?
...
\begin{enumerate}
 \item separation of variables to get two ODEs

 Substitute $u(x,t) = X(x)T(t)$ into PDE, transform to $\frac{X^{(n)}(x)}{X(x)} = \frac{T^{(m)}(t)}{T(t)} = -\lambda$

 ODEs: $X^{(n)}(x) + \lambda X(x) = 0 \quad T^{(m)}(t) + \lambda T(t) = 0$

 \item solve ODEs
 \item use \textbf{boundary conditions} as IVP for PDEs
 \item get general solution by superposition

 $$u(x,t) = \sum T_n(t) \cdot X_n(x)$$

 \item impose \textbf{initial conditions} to get coefficients of resulting (fourier series) solution\end{enumerate}
\end{recipe}
...
general solution, heat eq., Dirichlet bc.
$$
u(x,t)=\sum_{n=1}^{\infty} b_n\,e^{-k(n\pi/L)^2 t}\,
\sin\!\left(\frac{n\pi x}{L}\right)
$$

general solution, heat eq., Neumann bc.
$$
u(x,t)=a_0+\sum_{n=1}^{\infty} a_n\,e^{-k(n\pi/L)^2 t}\,
\cos\!\left(\frac{n\pi x}{L}\right)
$$


general solution, wave eq., dirichlet bc.
$$
...
general solution, wave eq., Neumann bc.
$$
u(x,t)=\frac{A_0+B_0 t}{2}+
\sum_{n=1}^{\infty}
\cos\!\left(\frac{n\pi x}{L}\right)
\left[
A_n\cos\!\left(\frac{n\pi c}{L}t\right)
...
\warning{Dirichlet starts at $1$, Neumann at $0$ (constant mode for heat/wave)}
...
$$
\Delta u = W_{rr} + \frac 1r W_r + \frac{1}{r^2} W_{\sigma\sigma} = 0
$$

\hl{TODO} complete

\hl{TODO} define harmonic functions

\section{Maximum principles}
harmonic function $u$, $\Delta u = 0$
\begin{enumerate}
 \item \textbf{Weak maximum principle:} $D$ bounded domain, $u\in C^2(D) \cap C(\overline{D})$, $u$ harmonic $\implies \max_D u = \max_{\partial D}u$
 \item \textbf{Strong maximum principle:} $\Delta u = 0$ in $D$ \\
...
Let $D$ be a bounded domain and let $u(x,y) \in C^2(D)\cap C(\overline{D})$
be harmonic in $D$. The maximum (and minimum) of $u$ in $\overline{D}$
is achieved on the boundary $\partial D$:
...
If $u$ is harmonic in $D$ and $B_R(x_0,y_0)\subset D$, then
\[
u(x_0,y_0)
= \frac{1}{2\pi R} \int_{\partial B_R(x_0,y_0)} u\,ds
